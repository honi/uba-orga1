...
type_MP = ["MSTR", "MLOAD", "MMOV", "MADD"] # MemoryPlus

opcodes = {"ADD" : 1, "ADC" : 2, "SUB" : 3, "AND"  : 4, "OR"  : 5, "XOR" : 6, "CMP" : 7, "MOV" : 8,
           "SIG" : 9, "NEG" : 10, "MIX" : 11,
           "MSTR": 12, "MLOAD": 13, "MMOV": 14, "MADD": 15,
...
def parseInstructions(instructions,labels):
    ...
    for i in instructions:
        ...
        try:
            ...
            # Instrucciones de MemoryPlus
            elif i[0] in type_MP:
                if i[0]=="MSTR" and i[1]=="[" and i[2][0]=="R" and i[3]=="]" and i[4]==",":
                    # MSTR [Rx], M  || MemPlus[Rx] <= M
                    n = buidInst({"O":opcodes[i[0]], "X":reg2num(i[2]), "M":mem2num(i[5],labels)})
                    appendParse(parseBytes,parseHuman,i,n)
                elif i[0] in ["MMOV", "MADD"] and i[1]=="[" and i[2][0]=="R" and i[3]=="]" and i[4]=="," and i[5]=="[" and i[6][0]=="R" and i[7]=="]":
                    # MMOV [Rx], [Ry]  || MemPlus[Rx] <= MemPlus[Ry]
                    # MADD [Rx], [Ry]  || MemPlus[Rx] <= MemPlus[Rx] + MemPlus[Ry]
                    n = buidInst({"O":opcodes[i[0]], "X":reg2num(i[2]), "Y":reg2num(i[6])})
                    appendParse(parseBytes,parseHuman,i,n)
                elif i[0]=="MLOAD" and i[1][0]=="R" and i[2]=="," and i[3]=="[" and i[4][0]=="R" and i[5]=="]":
                    # MLOAD [Rx], Ry  || MemPlus[Rx] <= MemPlus[Ry]
                    n = buidInst({"O":opcodes[i[0]], "X":reg2num(i[1]), "Y":reg2num(i[4])})
                    appendParse(parseBytes,parseHuman,i,n)
                else:
                    raise ValueError("Error: Invalid instruction \"" + i[0] + "\"")
                    break
...
signals={ "RB_enIn"           :  0,
          ...
          "MEM_OP"            : 12,
          "RESERVED_MEM_OP_1" : 13,
          "RESERVED_MEM_OP_2" : 14,
          "JC_microOp"        : 16,
          "JZ_microOp"        : 17,
          "JN_microOp"        : 18,
          "I_microOp"         : 19,
